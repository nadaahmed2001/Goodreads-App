...
const HomePage = () => {
  ...
  const handlePrevAuthor = () => {
    setCurrentAuthorIndex(prevIndex => prevIndex === 0 ? authors.length - 1 : prevIndex - 1);
  };

  const handleNextAuthor = () => {
    setCurrentAuthorIndex(prevIndex => prevIndex === authors.length - 1 ? 0 : prevIndex + 1);
  };
          ...
          <section className="PopularAuthorsSection">
            ...
            {currentAuthor && (
              <Card sx={{ maxWidth: 500 }} className="Card-home">
                <CardMedia sx={{ height: 500 }} image={currentAuthor.image} title={currentAuthor.name} />
                <CardContent>
                  <Typography gutterBottom variant="h5" component="div">{currentAuthor.name}</Typography>
